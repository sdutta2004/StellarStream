/**
 * deploy.js - Deploy both stellar_stream and stream_token Soroban contracts to Stellar Testnet,
 * initialize them, and link them on-chain.
 * Uses @stellar/stellar-sdk v13 + Horizon for TX polling.
 *
 * StellarStream — Real-Time Payment & Salary Vesting Protocol
 * Deployer Wallet: GBJ2JX6FPAW3E6ZCSGKDRXDGCMJJWRSYQCJLZ7APX2ICKIYWAMDON7QF
 *
...
// Default deployer address (Freighter wallet for StellarStream)
const DEFAULT_DEPLOYER_ADDRESS = "GBJ2JX6FPAW3E6ZCSGKDRXDGCMJJWRSYQCJLZ7APX2ICKIYWAMDON7QF";

// WASM build artifacts — stellar_stream and stream_token
const streamTokenWasmPath = path.join(
  __dirname,
  "..",
  "target",
...
const stellarStreamWasmPath = path.join(
  __dirname,
  "..",
  "target",
  "wasm32-unknown-unknown",
...
async function main() {
  ...
  // 1. Upload stream_token WASM & deploy instance
  console.log("\n--- Deploying STRM Protocol Token Contract ---");
  const streamTokenWasmHash = await uploadWasm(rpc, deployerKeypair, streamTokenWasmPath);
  const streamTokenSalt = Buffer.alloc(32, 1); // salt = 1 for stream_token
  const streamTokenId = await createContractInstance(rpc, deployerKeypair, streamTokenWasmHash, streamTokenSalt);

  // 2. Upload stellar_stream WASM & deploy instance
  console.log("\n--- Deploying StellarStream Payment Stream Contract ---");
  const stellarStreamWasmHash = await uploadWasm(rpc, deployerKeypair, stellarStreamWasmPath);
  const stellarStreamSalt = Buffer.alloc(32, 0); // salt = 0 for stellar_stream
  const stellarStreamId = await createContractInstance(rpc, deployerKeypair, stellarStreamWasmHash, stellarStreamSalt);

  // 3. Initialize StellarStream Contract (sets deployer as admin)
  console.log("\n--- Initializing StellarStream Payment Stream Contract ---");
  ...
  // 4. Initialize STRM Protocol Token Contract (sets StellarStream contract ID as admin)
  console.log("\n--- Initializing STRM Protocol Token Contract ---");
  await invokeContract(
    rpc,
    deployerKeypair,
    streamTokenId,
    "initialize",
    [
      new Address(stellarStreamId).toScVal(),
      nativeToScVal("StellarStream Reward"),
      nativeToScVal("STRM"),
    ]
  );

  // 5. Connect STRM Protocol Token to StellarStream Contract
  console.log("\n--- Linking STRM Token to StellarStream Contract ---");
  await invokeContract(
    rpc,
    deployerKeypair,
    stellarStreamId,
    ...
    [
      new Address(deployerKeypair.publicKey()).toScVal(),
      new Address(streamTokenId).toScVal(),
    ]
  );
  ...
  // Update stream contract ID (new key name)
  if (envContent.includes("NEXT_PUBLIC_STREAM_CONTRACT_ID=")) {
    envContent = envContent.replace(
      /NEXT_PUBLIC_STREAM_CONTRACT_ID=.*/,
      `NEXT_PUBLIC_STREAM_CONTRACT_ID=${stellarStreamId}`
    );
  } else if (envContent.includes("NEXT_PUBLIC_CROWDFUND_CONTRACT_ID=")) {
    envContent = envContent.replace(
  ...
  // Update STRM token contract ID (new key name)
  if (envContent.includes("NEXT_PUBLIC_STRM_TOKEN_CONTRACT_ID=")) {
    envContent = envContent.replace(
      /NEXT_PUBLIC_STRM_TOKEN_CONTRACT_ID=.*/,
      `NEXT_PUBLIC_STRM_TOKEN_CONTRACT_ID=${streamTokenId}`
    );
  } else if (envContent.includes("NEXT_PUBLIC_REWARD_TOKEN_CONTRACT_ID=")) {
    envContent = envContent.replace(
  ...
  readmeContent = readmeContent.replace(
    /\| \*\*Contract ID\*\* \| .*\|/,
    `| **StellarStream Contract ID** | \`${stellarStreamId}\` |\n| **STRM Token Contract ID** | \`${streamTokenId}\` |`
  );

  readmeContent = readmeContent.replace(
    /\[View Contract on Stellar Expert\]\([^)]+\)/,
    `[View Contract on Stellar Expert](https://stellar.expert/explorer/testnet/contract/${stellarStreamId})`
  );
  ...
  console.log(`\n${"═".repeat(65)}`);
  console.log(`STELLARSTREAM CONTRACT ID      : ${stellarStreamId}`);
  console.log(`STRM TOKEN CONTRACT ID         : ${streamTokenId}`);
  console.log(`DEPLOYER ADDRESS               : ${deployerKeypair.publicKey()}`);
  console.log(`DEFAULT FREIGHTER WALLET       : ${DEFAULT_DEPLOYER_ADDRESS}`);
  console.log(`EXPLORER LINK                  : https://stellar.expert/explorer/testnet/contract/${stellarStreamId}`);
  console.log(`${"═".repeat(65)}\n`);
}

main().catch((err) => {
  console.error("\n❌ Fatal error during StellarStream deployment:", err.message || err);
  process.exit(1);
});
